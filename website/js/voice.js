...
async function selectAction() {
  ...
  if (Array[0] == "도움말") {
    if (Array[1] == "스캔") {
      ...
      ttsSpeak(
        "음량을 조절합니다. 명령어는 '음량 조절, 숫자'입니다. 숫자의 범위는 0부터 100까지입니다."
      );
    } else
      ttsSpeak(
    ...
    if (Array[1] == "등록") {
      ...
      for (let i in allergies) {
        if (Array[3] == allergies[i]) {
          ...
          ttsSpeak(
            "등록 불가능한 알레르기 식품입니다. '도움말 알레르기' 명령어로 사용법을 확인해 보세요."
          );
        }
      }
    } else if (Array[1] == "정보") {
      fetch(server_ip + "/getUser")
        ...
        .then((data) => {
          data.forEach((user) => {
            ttsSpeak(
              `${user.id}번에 저장된 정보는 "이름" ${user.name} "알레르기" ${user.allergy}입니다.`
            );
          });
        });
    } else if (Array[1] == "삭제") {
      ...
      ttsSpeak(
        "잘못된 명령어입니다. '도움말 알레르기'로 사용법을 확인해보세요."
      );
  } else {
